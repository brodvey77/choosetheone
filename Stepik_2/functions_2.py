...
#
#
# print(reduce(lambda x, y: x + ' ' + y, sorted(sorted(data), key=len)))
...
# print(max(filter(lambda x: x if type(x) == int else 0, mixed_list)))
#
...
#
#
# a = sorted(filter(lambda x: str(x) if type(x) == int else 0, mixed_list))
# b = sorted(filter(lambda x: x if type(x) == str else 0, mixed_list))
# c = a+b
# print(*c)
